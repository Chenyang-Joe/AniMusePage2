...
async function loadAnimal(filename, xOffset) {
  ...
  const MESH_EMISSIVE_INTENSITY = 0.3;
  ...
  meshModel.traverse((obj) => {
    if (obj.isMesh) {
      ...
      // Self-illuminate using the base texture — lifts dark albedos out of the
      // shadows without washing out highlights (emissive == texture, not a flat
      // color). Matches the scene_teaser pattern.
      if (obj.material.map && !obj.material.emissiveMap) {
        obj.material.emissiveMap = obj.material.map;
        obj.material.emissive = new THREE.Color(0xffffff);
        obj.material.emissiveIntensity = MESH_EMISSIVE_INTENSITY;
      }
